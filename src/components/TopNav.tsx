...
const isCurrentPath = (currentPath: string, itemPath: string): boolean => {
    ...
    const normalizedCurrent = currentPath.endsWith('/') && currentPath.length > 1
        ...
        : currentPath;
    const normalizedItem = itemPath.endsWith('/') && itemPath.length > 1
        ? itemPath.slice(0, -1)
        : itemPath;
...
export const TopNav: React.FC = () => {
    ...
    return (
        // 确保粘性定位 (sticky top-0) 和高 z-index
        <div className="border-b bg-background sticky top-0 z-1000 shadow-md">
            <div className="container mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">

                {/* Logo/主页链接 */}
                <Link to="/" className="text-xl font-bold text-primary mr-6 tracking-wider">
                    应用名称
                </Link>
                    ...
                    <NavigationMenuList>
                        {menuData.map(item => {
                            ...
                            const isActive = isCurrentPath(currentPath, item.href) ||
                                (item.children && item.children.some(child => isCurrentPath(currentPath, child.href)));
